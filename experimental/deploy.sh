...
error_handler() {
    ...
    if [[ -n "${PREFIX:-}" ]]; then
        echo "LAST_FAILED_OPERATION=deploy" >> "${SCRIPT_DIR}/last_operation.env" 2>/dev/null || true
...
log_warn "For a redeployment, please make sure you change the prefix and run the delete.sh script prior to redeployment."
echo
...
if promptUser "Continue with prerequisites setup?"; then
  ...
  for stage_dir in "${SCRIPT_DIR}"/../fast/stages-aw/*/; do
    if [[ -d "$stage_dir" ]]; then
      ...
      if [[ -n "$old_prefix_patterns" ]] || [[ -n "$backup_prefix_patterns" ]]; then
        old_files_found=true
        stages_with_old_files+=("$stage_name")
        # Collect unique old prefixes found
        for prefix in $old_prefix_patterns $backup_prefix_patterns; do
    ...
    if handle_prompt "Would you like to clean up ALL old deployment files (keeping only '${current_prefix}') across all stages?"; then
      ...
      for stage_dir in "${SCRIPT_DIR}"/../fast/stages-aw/*/; do
        if [[ -d "$stage_dir" ]]; then
          ...
          find . -name "*.tf" -exec grep -l '[a-z0-9]\+-prod-iac-core-[0-9]' {} \; 2>/dev/null | while read -r file; do
            if ! grep -q "${current_prefix}-prod-iac-core" "$file" 2>/dev/null; then
              ...
              if [[ "$file" == *"providers.tf" ]]; then
                ...
                sed -i "s/${old_prefix}-prod-iac-core/${current_prefix}-prod-iac-core/g" "$file" 2>/dev/null || true
                sed -i "s/${old_prefix}-prod-bootstrap/${current_prefix}-prod-bootstrap/g" "$file" 2>/dev/null || true
                sed -i "s/${old_prefix}-prod-resman/${current_prefix}-prod-resman/g" "$file" 2>/dev/null || true
                sed -i "s/${old_prefix}-security/${current_prefix}-security/g" "$file" 2>/dev/null || true
              else
          ...
          find . -name "*.json" -exec grep -l '[a-z0-9]\+-prod-iac-core-[0-9]' {} \; 2>/dev/null | while read -r file; do
            if ! grep -q "${current_prefix}-prod-iac-core" "$file" 2>/dev/null; then
              old_prefix=$(grep -o '[a-z0-9]\+-prod-iac-core-[0-9]' "$file" | head -1 | cut -d'-' -f1)
              log_info "  Removing old tfvars: $file (will be regenerated with $current_prefix)"
...
EOF
  ...
  if [[ "$state_migrated" == "true" ]]; then
    ...
    # Try terraform apply first, if it fails we'll check for BigQuery errors
    if terraform apply -var bootstrap_user="$bootstrap_user"; then
      log_info "First terraform apply completed successfully"
    else
      # Terraform failed, let's check if it's a BigQuery constraint error
      ...
      # Check for both restrictServiceUsage AND bigquery.googleapis.com anywhere in the output
      # Use bash matching to avoid pipefail SIGPIPE issues with grep -q and large output
      if [[ "$terraform_output" == *"restrictServiceUsage"* ]] && [[ "$terraform_output" == *"bigquery.googleapis.com"* ]]; then
        log_warn "Detected BigQuery constraint error - Assured Workloads folder needs BigQuery allowlist update"
        # echo "$terraform_output" # Suppress verbose output on successful detection

        if handle_prompt_if "Would you like to run the BigQuery allowlist fix and retry terraform apply?"; then
          log_info "Running BigQuery allowlist script..."
          if [[ -f "${SCRIPT_DIR}/allow_bq.sh" ]]; then
            if "${SCRIPT_DIR}/allow_bq.sh"; then
              ...
              if ! terraform apply -var bootstrap_user="$bootstrap_user"; then
                log_error "Terraform apply still failed after BigQuery fix"
                exit 1
              fi
              log_info "First terraform apply completed successfully after BigQuery fix"
            else
              log_error "BigQuery allowlist script failed"
              exit 1
            fi
          else
            log_error "allow_bq.sh script not found"
            exit 1
          fi
        else
          log_error "First terraform apply failed due to BigQuery constraints"
  ...
  handle_prompt "Would you like to import recommended org policies?" "./import.sh" || true

  # Terraform Apply #3 (after state migration, without bootstrap user)
  handle_prompt "Would you like to run the third terraform apply (without the bootstrap user)?" "terraform apply"
fi
...
EOF
  ...
  handle_prompt "Would you like to pull the remote tfvars files created in Stage 0?" "${cmd[@]}"

  handle_prompt "Would you like to perform the terraform init?" "terraform init"
  handle_prompt "Would you like to perform the terraform apply?" "terraform apply"
...
if promptUser "Stage 2 - Networking -"; then
        ...
        if terraform apply; then
          ...
          if [[ $attempt -lt $max_attempts ]]; then
            ...
            if handle_prompt_if "Would you like to try one more manual attempt?"; then
              terraform apply && apply_success=true
            fi
            break
          fi
        fi
  ...
  ########### IL4/IL5 ###########
    ...
    handle_prompt "Would you like to perform the targeted terraform apply?" "${cmd[@]}"
