...
- In GKE, the allocation of the nodes is done as per the Zone. For example, if there are 3 Zones and the initial node allocation per Zone is 1, then the total number of Nodes in the cluster shall be initial node allocation per zone times the total number of Zone 
